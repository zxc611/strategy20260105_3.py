        ...
        try:
            ...
            for fn in ('get_bars', 'get_klines', 'fetch_bars', 'bars'):
                ...
                if callable(f):
                    try:
                        try:
                            return f(exchange=exchange, instrument_id=instrument_id, period=period, count=count)
                        except TypeError:
                            # 位置参数尝试
                            return f(exchange, instrument_id, period, count)
                    except Exception:
                        continue
            raise AttributeError('no compatible get_bars function found on proxied SDK')
        except Exception:
